    ...
    builder: (_) => AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            detalleItem('Usuario', forzado.usuarioCreacion ?? 'No disponible'),
            detalleItem('Nombre del Proyecto', forzado.proyectoDescripcion ?? 'No disponible'),
            detalleItem('Centro', forzado.tagCentroDescripcion ?? 'No disponible'),
            detalleItem('Descripción', forzado.descripcion ?? 'No disponible'),
            detalleItem('Disciplina', forzado.disciplinaDescripcion ?? 'No disponible'),
            detalleItem('Turno', forzado.turnoDescripcion ?? 'No disponible'),
            detalleItem('Interlock Seguridad', forzado.interlock?.toString() ?? 'No disponible'),
            detalleItem('Responsable', forzado.responsableNombre ?? 'No disponible'),
            detalleItem('Riesgo', forzado.riesgoDescripcion ?? 'No disponible'),
            detalleItem('Solicitante', forzado.solicitante ?? 'No disponible'),
            detalleItem('Aprobador', forzado.aprobador ?? 'No disponible'),
            detalleItem('Ejecutor', forzado.ejecutor ?? 'No disponible'),
            detalleItem('Tipo de Forzado', forzado.tipoForzadoDescripcion ?? 'No disponible'),
            detalleItem('Motivo de rechazo', forzado.motivoRechazoDescripcion ?? 'No disponible'),
          ],
        ),
      ),
      actions: [
        ...
        return value.loading
            ...
            ListView.separated(
              ...
              separatorBuilder: (BuildContext context, int index) {
                ...
                return forzado.estado?.toUpperCase() != 'APROBADO-FORZADO' ? SizedBox.shrink() :Divider();
              },
              itemBuilder: (BuildContext context, int index) {
                ForzadoApprove  forzado = value.listForzados[index];
              
              
                return forzado.estado?.toUpperCase() != 'APROBADO-FORZADO'? SizedBox.shrink() :  Card(
                    elevation: 4,
                    margin:
